...
def log_history(filename, action, user):
    """Log an operation to the file's history"""
    if filename not in file_history:
        file_history[filename] = []
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    file_history[filename].append(f"[{timestamp}] User '{user}' -> {action}")
        ...
        while True:
            ...
            try:
                ...
                # File operations
                    ...
                    log_history(filename, "Created file", current_user)
                    ...
                    log_history(filename, "Uploaded/Overwritten file", current_user)
                    ...
                    if os.path.exists(old_path):
                        ...
                        # Mutam istoricul pe noul nume
                        if old_name in file_history:
                            file_history[new_name] = file_history.pop(old_name)
                        log_history(new_name, f"Renamed from {old_name}", current_user)
                    ...
                    if os.path.exists(filepath):
                        with open(filepath, 'r') as f:
                            content = f.read()
                        log_history(filename, "Read file content", current_user)
                    ...
                    if os.path.exists(filepath):
                        with open(filepath, 'r') as f:
                            content = f.read()
                        log_history(filename, "Downloaded file", current_user)
                        response = {'status': 'success', 'content': content, 'message': 'File sent successfully'}
                    else:
                    ...
                    if os.path.exists(filepath):
                        with open(filepath, 'w') as f:
                            f.write(content)
                        log_history(filename, "Edited file", current_user)
                    ...
                    if filename in file_history and file_history[filename]:
                        history_content = "\n".join(file_history[filename])
                        response = {'status': 'success', 'history': history_content}
                    else:
                        response = {'status': 'error', 'message': f'No operation history found for {filename}'}

                elif command == 'list_files':
                    files = os.listdir(FILES_DIR)
                    # Ascundem posibile fisiere ascunse de la macOS (ex: .DS_Store)
