...
while True:
    ...
    if comanda in ['PUBLISH', 'DELETE', 'LIST']:
        if not este_conectat:
            print("EROARE LOCALA: Nu poti trimite comanda. Nu esti conectat! (Foloseste CONNECT)")
            continue
    ...
    if comanda == 'CONNECT':
        ...
        if not argument or not argument.isdigit():
            print("EROARE LOCALA: Comanda DELETE necesita un ID numeric valid. (ex: DELETE 1)")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
